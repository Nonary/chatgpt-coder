...
Select **AI summary** to package the repository's staged, unstaged, and untracked changes through the same Git-bundle ZIP workflow used for coding tasks. Because it runs that whole pipeline, Patchwork opens the summary's task view as it starts, so the mirrored conversation, the activity log, and the run status are visible while it works rather than only once it finishes. Patchwork uses the saved prompt named **Git Summary** when one exists in the local Prompt library. Otherwise it uses the built-in Git Changes Review + Conventional Commit prompt. The generated message is placed into the commit editor without changing or staging files.
...
Patchwork owns the model and reasoning choice for every chat, and mirrors it into a picker inside the browser page so it stays changeable there. The value Patchwork wrote is never allowed to speak back as if it were a user choice: only a selection actually made in that page picker, for that chat, replaces the configuration a task was created with. Starting a new chat carries the current choice forward instead of reverting to the default model.

A chat that has never been sent owns no conversation, so Patchwork shows it empty rather than reading whatever the shared browser still has on screen. It follows the page live from the first message, before ChatGPT has assigned the conversation a route, and ignores the outgoing transcript until the page has confirmed it cleared.
...
4. Patchwork creates a ZIP containing `AGENTS.md`, `TASK.md`, `manifest.json`, selected skill directories under `skills/`, user-provided files under `attachments/`, and the selected repository or coding tree Git bundles under `repositories/`. Git submodules are bundled recursively under each repository entry and described in `manifest.json`, so the task upload carries their object history without requiring a remote fetch. Already-compressed bundles are stored directly in the ZIP; they are not redundantly compressed a second time.
5. Patchwork opens the selected ChatGPT destination in the same persistent browser, injects the instructions, attaches the ZIP package plus any user-selected reference files, waits for the attachments to finish processing, and clicks ChatGPT's Send button. The browser stays hidden on task pages while Patchwork mirrors the conversation into its native chat UI. The mirror keeps ChatGPT's rendered Markdown structure — headings, nested lists, block quotes, tables, links, and fenced code with its language and a copy action — and rebuilds it as real elements from text alone, never as markup. A transcript already scrolled to the bottom follows the stream; one scrolled up to re-read something is left where the reader put it.
...
The automation adapter intentionally targets a small set of semantic browser controls: the prompt composer, attachment input, Send button, generated-file links, project controls, New chat control, and blocking limit notices. Task, project, and chat switches use ChatGPT's in-app router and block document navigations so the one authenticated WebContents stays alive; a guarded document load is used only to recover when ChatGPT fails to expose a fresh-chat route.

One transcript reader in `src/main/chatgpt-transcript.js` serves both update paths, so the native chat cannot disagree with itself depending on which path produced a snapshot. The browser page preload does not read the conversation; it observes mutations and reports only that the rendered transcript changed, and Patchwork then reads the page once through that shared reader. Reads are coalesced, so a fast stream produces a steady sequence of complete snapshots rather than a backlog of stale ones. A slower serial fallback poll and the background result monitor cover missed page events and exact result filenames.

Because there is exactly one authenticated browser, every submitted task takes turns on the same route, and the background monitor is the only place that decides whose turn it is. The conversation currently shown in the native chat is served first, a live stream is never interrupted for a background status check, and a borrowed route is always handed back to the conversation on screen before the sweep ends. The same monitor dismisses ChatGPT's known conversation-history request-limit dialog through its semantic test ID and “Got it” action, with a narrow text-and-role fallback for other limit dialogs, including ones rendered as ordinary dialogs rather than alerts. Dismissal runs whether or not the browser is on screen, because a blocking notice swallows clicks on the composer, Send, and New chat either way. Every fallback container still has to match the limit-notice text, so it never closes unrelated prompts. Clicking a task hides the browser before navigating to its saved conversation, so browser navigation remains an implementation detail rather than the task UI. Because page markup can change, selector failures stop safely and leave the single persistent ChatGPT session available for manual recovery.
