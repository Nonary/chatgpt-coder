...
3. Describe the software task, optionally select local reference files, and choose where ChatGPT should launch it. A task can use a normal new chat, an existing ChatGPT project, or a newly created project. Patchwork copies attachments into the task record and embeds them in the task package under `attachments/`.
4. Patchwork creates a ZIP containing `AGENTS.md`, `TASK.md`, `manifest.json`, user-provided files under `attachments/`, and the coding tree's full-history Git bundle under `repositories/`. Already-compressed bundles are stored directly in the ZIP; they are not redundantly compressed a second time.
