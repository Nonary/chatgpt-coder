...
type ChatMessage = { id?: string; role: 'user' | 'assistant'; text: string; kind?: 'message' | 'reasoning'; createdAt?: number | string; status?: string; endTurn?: boolean };
...
function CodeBlock({ value }: { value: string }) {
  ...
  return <div className="code-block"><Button type="button" variant="ghost" className="code-copy" onClick={copy}>{copied ? 'Copied' : 'Copy'}</Button><pre><code>{value}</code></pre></div>;
}

function Markdown({ value }: { value: string }) {
  const blocks = value.split(/```/g);
  return <div className="markdown">{blocks.map((block, index) => index % 2 === 1
    ? <CodeBlock key={index} value={block.replace(/^\w+\n/, '')} />
    : block.split(/\n{2,}/g).map((paragraph, paragraphIndex) => paragraph
      ? <p key={`${index}-${paragraphIndex}`}>{paragraph.split(/(`[^`]+`)/g).map((part, partIndex) => part.startsWith('`') && part.endsWith('`')
        ? <code key={partIndex}>{part.slice(1, -1)}</code> : part)}</p>
      : null))}</div>;
}
...
function Sidebar({ route, onRoute, onTaskSelect, collapsed, setCollapsed, tasks, conversations, onNewTask, onNewChat, theme, setTheme, authenticated, onOpenSession }: { route: Route; onRoute: (route: Route) => void; onTaskSelect: (task: Task) => void; collapsed: boolean; setCollapsed: (value: boolean) => void; tasks: Task[]; conversations: Conversation[]; onNewTask: () => void; onNewChat: () => void; theme: Theme; setTheme: (value: Theme) => void; authenticated: boolean; onOpenSession: () => void }) {
  ...
  return <aside className={`sidebar ${collapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <button type="button" className="session-button" onClick={onOpenSession} title={collapsed ? 'Open ChatGPT' : undefined}><span className={`connection-dot ${authenticated ? '' : 'offline'}`} />{!collapsed && <span>{authenticated ? 'ChatGPT connected' : 'Sign in to ChatGPT'}</span>}</button>
  </aside>;
}
...
function Home({ repositories, trees, projects, promptIds, skillIds, attachments, setAttachments, preferredTreeId, onAddRepository, onCreateTask, onOpenSkills, onOpenPrompts }: { repositories: Repository[]; trees: Tree[]; projects: AnyRecord[]; promptIds: string[]; skillIds: string[]; attachments: AnyRecord[]; setAttachments: (items: AnyRecord[]) => void; preferredTreeId?: string; onAddRepository: () => void; onCreateTask: (input: AnyRecord) => Promise<void>; onOpenSkills: () => void; onOpenPrompts: () => void }) {
  ...
  useEffect(() => { if (!repositoryPath && repositories[0]) setRepositoryPath(repositories[0].path); }, [repositories, repositoryPath]);
  useEffect(() => { setModel(localStorage.getItem('patchwork.task-model') || 'default'); setReasoning(localStorage.getItem('patchwork.task-reasoning') || 'default'); setProjectId(localStorage.getItem('patchwork.task-project') || ''); setRepositoryPath(preferredTree?.repositoryPath || localStorage.getItem('patchwork.task-repository') || repositories[0]?.path || ''); if (!preferredTreeId) setTreeId(localStorage.getItem('patchwork.task-tree') || ''); setSettingsLoaded(true); }, []);
  useEffect(() => { if (!settingsLoaded) return; localStorage.setItem('patchwork.task-model', model); localStorage.setItem('patchwork.task-reasoning', reasoning); localStorage.setItem('patchwork.task-project', projectId); localStorage.setItem('patchwork.task-repository', repositoryPath); localStorage.setItem('patchwork.task-tree', treeId); }, [settingsLoaded, model, reasoning, projectId, repositoryPath, treeId]);
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); const repository = repositories.find((item) => item.path === repositoryPath); if (!taskText.trim() || !repository) { setError(repository ? 'Describe the task before continuing.' : 'Choose one repository before creating a task.'); return; } setBusy(true); setError(''); try { await onCreateTask({ repositories: [repository], taskText: taskText.trim(), model, reasoningMode: reasoning, treeId: treeId || null, createTree, treeName, skillIds, promptIds, attachments, autoApply: true, chatgptProject: projects.find((project) => project.id === projectId) || null }); setTaskText(''); setAttachments([]); } catch (e) { setError(errorMessage(e)); } finally { setBusy(false); } };
  return <div className="page page-home"><div className="hero"><div><div className="eyebrow">WORKSPACE</div><h2>What are you building?</h2><p>Describe a coding task and Patchwork will prepare a validated Git bundle for ChatGPT.</p></div><div className="hero-orb">✦</div></div>
    ...
    <div className="home-grid"><section className="panel"><div className="panel-heading"><div><div className="eyebrow">CONTEXT</div><h3>Repositories</h3></div><Button variant="ghost" onClick={onAddRepository}>＋ Add</Button></div>{repositories.length ? repositories.map((repo) => <div className="repo-row" key={repo.path}><span className="repo-icon">⌘</span><div><strong>{repo.name || repo.path.split('/').pop()}</strong><small>{repo.path}</small></div><span className={repo.dirty ? 'status-warning' : 'status-success'}>{repo.dirty ? 'Changes' : 'Clean'}</span></div>) : <Empty title="No repository selected" copy="Add a Git repository to give ChatGPT context." action={<Button onClick={onAddRepository}>Add repository</Button>} />}</section>
      <section className="panel"><div className="panel-heading"><div><div className="eyebrow">CONFIGURATION</div><h3>Task settings</h3></div></div><label>Repository<select value={repositoryPath} onChange={(e) => setRepositoryPath(e.target.value)}>{repositories.map((repo) => <option value={repo.path} key={repo.path}>{repo.name || repo.path}</option>)}</select></label><div className="field-grid"><label>Model<select value={model} onChange={(e) => setModel(e.target.value)}><option value="default">ChatGPT default</option><option value="sol">GPT-5.6 Sol</option><option value="luna">GPT-5.6 Luna</option></select></label><label>Reasoning<select value={reasoning} onChange={(e) => setReasoning(e.target.value)}><option value="default">Model default</option><option value="instant">Instant</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></label></div><label>Target<select value={createTree ? '__new__' : treeId} onChange={(e) => { setCreateTree(e.target.value === '__new__'); setTreeId(e.target.value === '__new__' ? '' : e.target.value); }}><option value="">Current repository</option>{trees.map((tree) => <option key={tree.id} value={tree.id}>{tree.name || tree.id}</option>)}<option value="__new__">Create a new coding tree…</option></select></label>{createTree && <label>New tree name<input value={treeName} onChange={(e) => setTreeName(e.target.value)} placeholder="Feature name" /></label>}<label>ChatGPT project<select value={projectId} onChange={(e) => setProjectId(e.target.value)}><option value="">New chat</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></label><div className="selection-summary">{promptIds.length ? `${promptIds.length} saved prompt${promptIds.length === 1 ? '' : 's'} selected · ` : ''}{skillIds.length ? `${skillIds.length} skill${skillIds.length === 1 ? '' : 's'} selected` : 'No skills selected'}</div></section></div>
  </div>;
}
...
function ChatWorkspace({ conversations, activeChat, awaitingReply, historyError, threadError, onSelect, onRefresh, onSend, onStop, onNew, model, setModel }: { conversations: Conversation[]; activeChat: Chat | null; awaitingReply: boolean; historyError: string; threadError: string; onSelect: (id: string) => void; onRefresh: () => void; onSend: (message: string, attachments: AnyRecord[], reasoningMode: string) => Promise<void>; onStop: () => Promise<void>; onNew: () => void; model: string; setModel: (model: string) => void }) {
  ...
  return <div className="chat-workspace">
    ...
    <section className="chat-thread">
      ...
      <div className="messages">{activeChat?.messages?.length ? activeChat.messages.map((item, index) => <article className={`message message-${item.role} ${item.kind === 'reasoning' ? 'message-reasoning' : ''}`} key={item.id || index}>{item.kind === 'reasoning' ? <details className="reasoning-card" open={working && index === activeChat.messages!.length - 1}><summary><span className="reasoning-spark">✦</span>Reasoning summary</summary><Markdown value={item.text} /></details> : <><div className="message-author">{item.role === 'user' ? 'You' : 'ChatGPT'}</div>{item.role === 'assistant' ? <Markdown value={item.text} /> : <div className="message-bubble">{item.text}</div>}</>}</article>) : <Empty title="How can I help?" copy="Ask ChatGPT about a task, a codebase, or an idea." />}{threadError && <div className="inline-notice thread-notice">{threadError}</div>}{working && <div className="working-row" role="status"><i className="mini-spinner" aria-hidden="true" /><span>ChatGPT is working</span></div>}</div>
...
function TaskDetail({ task, trees, conversation, conversationLoading, conversationError, activity, onAction, onConflict, onDelete, onSetTarget, onRefreshConversation }: { task: Task | null; trees: Tree[]; conversation: Chat | null; conversationLoading: boolean; conversationError: string; activity: TaskActivity[]; onAction: (name: string) => Promise<void>; onConflict: () => void; onDelete: () => Promise<void>; onSetTarget: (treeId: string | null) => Promise<void>; onRefreshConversation: () => void }) {
  ...
  return <div className="page page-task"><div className="task-heading"><div><div className="eyebrow">TASK</div><h2>{firstLine(task.taskText)}</h2><p>{formatDate(task.createdAt)} · {statusLabel(task)}</p></div><div className="task-heading-actions"><Button onClick={() => onAction('openChatInSession')}>Open ChatGPT</Button><span className={`status-badge ${task.state}`}>{working && <i className="mini-spinner" aria-hidden="true" />}{statusLabel(task)}</span></div></div><div className="task-grid"><div className="task-primary"><section className="panel task-main"><div className="panel-heading"><div><div className="eyebrow">REQUEST</div><h3>Task instructions</h3></div><Button variant="danger" onClick={onDelete}>Delete</Button></div><Markdown value={text(task.taskText)} /><div className="task-actions">{task.state === 'prepared' && <Button variant="primary" onClick={() => onAction('submitTask')}>Submit to ChatGPT</Button>}{task.state === 'ready' && !task.summaryOnly && <Button variant="primary" onClick={() => onAction('applyTask')}>Apply changes</Button>}{task.state === 'ready' && task.summaryOnly && <Button variant="primary" onClick={() => onAction('useGitSummary')}>Use in Source Control</Button>}{task.state === 'conflicted' && <><Button variant="primary" onClick={() => onAction('retryApplyTask')}>Retry apply</Button><Button onClick={onConflict}>Resolve with ChatGPT</Button></>}{task.state === 'applied' && <Button variant="danger" onClick={() => onAction('rollbackTask')}>Roll back</Button>}<Button onClick={() => onAction('copyPrompt')}>Copy prompt</Button><Button onClick={() => onAction('revealPackage')}>Show package</Button><Button onClick={() => onAction('importResult')}>Import result</Button></div></section><section className="panel task-updates-panel"><div className="panel-heading"><div><div className="eyebrow">CHATGPT UPDATES</div><h3>Live task transcript</h3></div><Button variant="ghost" disabled={!task.conversationId || conversationLoading} onClick={onRefreshConversation}>{conversationLoading ? 'Refreshing…' : '↻ Refresh'}</Button></div>{conversationError && <div className="inline-notice">{conversationError}</div>}<div className="task-updates">{feed.map((item) => item.kind === 'activity' ? <div className="task-progress-row" key={item.id}><span className={working && item === feed.at(-1) ? 'mini-spinner' : 'activity-dot'} aria-hidden="true" /><div><strong>{item.activity.message}</strong><small>{activityLabel(item.activity.type)} · {formatDate(item.activity.createdAt)}</small></div></div> : item.message.kind === 'reasoning' ? <details className="reasoning-card task-reasoning" key={item.id} open={working && item === feed.at(-1)}><summary><span className="reasoning-spark">✦</span>Reasoning summary</summary><Markdown value={item.message.text} /></details> : <article className="task-update" key={item.id}><div className="task-update-heading"><span>ChatGPT</span>{item.message.createdAt && <small>{formatDate(timestampMilliseconds(item.message.createdAt))}</small>}</div><Markdown value={item.message.text} /></article>)}{working && <div className="working-row" role="status"><i className="mini-spinner" aria-hidden="true" /><span>ChatGPT is working</span></div>}{feed.length === 0 && !working && <Empty title={task.conversationId ? 'Waiting for an update' : 'Task not submitted yet'} copy={task.conversationId ? 'Visible replies, tool activity, and reasoning summaries appear here automatically.' : 'Conversation updates appear after ChatGPT accepts the task.'} />}</div></section></div><aside className="task-side"><section className="panel"><div className="eyebrow">APPLY TARGET</div><label>Destination<select value={task.treeId || ''} disabled={!canChangeTarget} onChange={(event) => onSetTarget(event.target.value || null)}><option value="">Original repository</option>{trees.map((tree) => <option value={tree.id} key={tree.id}>{tree.name || tree.id}</option>)}</select></label>{!canChangeTarget && <p className="panel-note">Applied tasks keep their recorded destination.</p>}</section><section className="panel"><div className="eyebrow">RESULT</div>{result ? <><h3>{result.summary || 'Validated result'}</h3><div className="patch-list">{(result.patches || []).map((patch: AnyRecord) => <div className="patch-item" key={patch.id || patch.name}><strong>{patch.name || patch.id}</strong><code>{patch.stat || 'Changes ready'}</code></div>)}</div></> : <Empty title="Waiting for ChatGPT" copy="The validated result will appear here when it is ready." />}</section></aside></div></div>;
}

function SourceControl({ repositories, onRefresh, onAdd, onCommit, onStage, onUnstage }: { repositories: Repository[]; onRefresh: (path?: string) => Promise<AnyRecord | null>; onAdd: () => void; onCommit: (repositoryPath: string, message: string) => Promise<void>; onStage: (repositoryPath: string, file: string) => Promise<void>; onUnstage: (repositoryPath: string, file: string) => Promise<void> }) {
  const [selected, setSelected] = useState(repositories[0]?.path || '');
  const [status, setStatus] = useState<AnyRecord | null>(null);
  const [message, setMessage] = useState('');
  const [diff, setDiff] = useState('');
  const [summaryBusy, setSummaryBusy] = useState(false);
  const refreshSelected = useCallback(async () => { if (selected) setStatus(await onRefresh(selected)); }, [selected, onRefresh]);
  useEffect(() => { refreshSelected().catch(() => {}); }, [refreshSelected]);
  useEffect(() => { if (!selected && repositories[0]) setSelected(repositories[0].path); }, [repositories, selected]);
  const changes = status?.changes || [];
  const preview = async (change: AnyRecord, staged: boolean) => { const result = await call<AnyRecord>('gitDiff', selected, change.path, staged); setDiff(result?.content || (result?.rows || []).map((row: AnyRecord) => `${row.before || ''} ${row.after || ''}`).join('\n')); };
  const changeRows = (staged: boolean) => changes.filter((change: AnyRecord) => Boolean(change[staged ? 'staged' : 'unstaged'])).map((change: AnyRecord) => <div className="change-row" key={`${staged ? 'staged' : 'unstaged'}-${change.path}`}><button type="button" className="change-open" onClick={() => preview(change, staged)}><span className={staged ? 'file-status staged' : 'file-status'}>{staged ? 'S' : 'M'}</span><span>{change.path}</span></button><Button type="button" variant="ghost" aria-label={`${staged ? 'Unstage' : 'Stage'} ${change.path}`} onClick={() => (staged ? onUnstage(selected, change.path) : onStage(selected, change.path)).then(refreshSelected)}>{staged ? '−' : '+'}</Button></div>);
  const summarize = async () => { setSummaryBusy(true); try { const result = await call<AnyRecord>('gitSummary', selected, null); setMessage(result.commitMessage || ''); } finally { setSummaryBusy(false); } };
  return <div className="page page-source"><div className="workspace-heading"><div><div className="eyebrow">DEVELOPMENT</div><h2>Source control</h2><p>Review, stage, and commit changes in your local workspace.</p></div><div className="heading-actions"><select value={selected} onChange={(event) => setSelected(event.target.value)}>{repositories.map((repo) => <option value={repo.path} key={repo.path}>{repo.name || repo.path}</option>)}</select><Button onClick={onAdd}>＋ Add repository</Button><Button variant="ghost" onClick={refreshSelected}>↻</Button></div></div>{repositories.length === 0 ? <Empty title="No repositories" copy="Add a repository to inspect its working tree." action={<Button onClick={onAdd}>Add repository</Button>} /> : <div className="source-grid"><section className="panel change-panel"><div className="commit-editor"><textarea rows={3} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Commit message" /><div className="source-action-row"><Button variant="primary" disabled={!selected || !message.trim()} onClick={() => onCommit(selected, message).then(() => { setMessage(''); return refreshSelected(); })}>Commit staged changes</Button><Button onClick={() => call('gitStageAll', selected).then(refreshSelected)}>Stage all</Button><Button onClick={() => call('gitUnstageAll', selected).then(refreshSelected)}>Unstage all</Button><Button disabled={summaryBusy || changes.length === 0} onClick={summarize}>{summaryBusy ? 'Generating…' : '✦ AI summary'}</Button></div></div><div className="change-group"><div className="panel-heading"><h3>Staged changes</h3><span>{status?.stagedCount || 0}</span></div>{changeRows(true)}{!changes.some((change: AnyRecord) => change.staged) && <div className="muted-block">No staged changes.</div>}</div><div className="change-group"><div className="panel-heading"><h3>Unstaged changes</h3><span>{status?.unstagedCount || 0}</span></div>{changeRows(false)}{!changes.some((change: AnyRecord) => change.unstaged) && <div className="muted-block">Working tree clean.</div>}</div><div className="change-group"><div className="panel-heading"><h3>Recent commits</h3></div>{(status?.history || []).slice(0, 8).map((commit: AnyRecord) => <div className="commit-history-row" key={commit.commit}><strong>{commit.subject}</strong><small>{commit.shortCommit} · {commit.author}</small></div>)}</div></section><section className="panel diff-panel"><div className="panel-heading"><h3>Diff preview</h3></div><pre className="diff-preview">{diff || 'Select a changed file to inspect its diff.'}</pre></section></div>}</div>;
}

function Trees({ trees, projects, onRefresh, onRemove, onMerge, onReveal, onNewTask, onResolve }: { trees: Tree[]; projects: AnyRecord[]; onRefresh: () => void; onRemove: (id: string) => void; onMerge: (id: string, project: AnyRecord | null) => void; onReveal: (id: string) => void; onNewTask: (tree: Tree) => void; onResolve: (tree: Tree) => void }) {
  const [project, setProject] = useState(''); return <div className="page"><div className="workspace-heading"><div><div className="eyebrow">WORKTREES</div><h2>Coding trees</h2><p>Keep independent tasks isolated until you are ready to merge them.</p></div><div className="heading-actions"><select value={project} onChange={(e) => setProject(e.target.value)}><option value="">Default ChatGPT project</option>{projects.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><Button onClick={onRefresh}>↻ Refresh</Button></div></div>{trees.length ? <div className="tree-grid">{trees.map((tree) => <article className="tree-card" key={tree.id}><div className="tree-card-heading"><span className="repo-icon">⑃</span><div><h3>{tree.name || tree.id}</h3><small>{tree.path}</small></div><span className={`status-badge ${tree.mergeState || tree.state || 'ready'}`}>{tree.mergeState || tree.state || 'ready'}</span></div><div className="tree-stats"><span><strong>{tree.taskCount || 0}</strong><small>Tasks</small></span><span><strong>{tree.commitCount || 0}</strong><small>Commits</small></span></div><div className="tree-actions"><Button onClick={() => onReveal(tree.id)}>Open folder</Button><Button onClick={() => onNewTask(tree)}>＋ Continue task</Button>{tree.mergeState === 'failed' ? <Button onClick={() => onResolve(tree)}>Resolve merge</Button> : <Button onClick={() => onMerge(tree.id, projects.find((item) => item.id === project) || null)}>Merge tree</Button>}<Button variant="danger" onClick={() => onRemove(tree.id)}>Discard</Button></div></article>)}</div> : <Empty title="No coding trees" copy="Create one from the New Task workspace when you need isolated work." />}</div>;
}
...
export default function App() {
  ...
  const [dialog, setDialog] = useState<'prompts' | 'skills' | 'conflict' | null>(null);
  ...
  const conversationIsWorking = useCallback((chat: Chat | null) => {
    ...
    if (chat.status === 'streaming') return true;
    const last = chat.messages?.at(-1);
    return last?.role === 'user' || last?.kind === 'reasoning';
  }, []);
  ...
  }, [appendTaskActivity, notify, refresh, refreshConversationList, refreshProjects, refreshSessionStatus]);
  ...
  const sendChat = async (message: string, chatAttachments: AnyRecord[], reasoningMode: string) => {
    ...
    const optimistic: Chat = { ...(activeChat || { id: pendingId, title: 'New chat' }), status: 'streaming', updateTime: Date.now(), messages: [...(activeChat?.messages || []), { id: `pending-message-${Date.now()}`, role: 'user', text: message }] };
